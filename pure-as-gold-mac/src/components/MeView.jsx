...
const MeView = ({ theme, onToggleTheme, tasks, todos, onImportReminders }) => {
    ...
    // --- LOGIC: Process Cached Data ---
    const processAndSync = (reminders, selectedIds) => {
        // Extract unique lists from reminders
        const lists = [];
        const listNames = new Set();
        reminders.forEach(r => {
            if (!listNames.has(r.list)) {
                listNames.add(r.list);
                lists.push({
                    id: r.list, // Use list name as ID
                    title: r.list,
                    color: '#6c7cdd'
                });
            }
        ...
        // Filter reminders by selected lists and pass to app
        // If IDs are provided, filter. If selection is empty, show nothing or default?
        // Let's assume empty selection = nothing synced.
        const selectedReminders = reminders.filter(r => selectedIds.includes(r.list));

        // Format for the main app
        const formatted = selectedReminders.map(r => ({
            id: r.id,
            title: r.title,
            tag: r.list,
            done: false,
        ...
        onImportReminders(formatted);
    };

    // --- LOGIC: Fetch Data from Mac ---
    const syncReminders = async () => {
        ...
        try {
            ...
            // Explicitly request/warm up permission
            console.log("Requesting permission...");
            const hasAccess = await window.electronAPI.requestRemindersAccess();

            if (!hasAccess) {
                alert("Permission denied. please enable Reminders access in System Settings > Privacy & Security > Reminders.");
                setIsImporting(false);
                return;
            }

            const macReminders = await window.electronAPI.fetchReminders();
            setAllRawReminders(macReminders);

            // If it's the first fetch, select all lists by default
            let currentSelection = selectedListIds;
            if (selectedListIds.length === 0 && macReminders.length > 0) {
                const uniqueLists = [...new Set(macReminders.map(r => r.list))];
                setSelectedListIds(uniqueLists);
                currentSelection = uniqueLists;
            }

            processAndSync(macReminders, currentSelection);
            setIsImporting(false);
        } catch (error) {
            console.error("Failed to sync:", error);
            setIsImporting(false);
            alert("Could not access Reminders. Please check System Settings > Privacy & Security > Reminders and try again.");
        }
    };
    ...
    const handleToggleReminders = async () => {
        if (!importReminders) {
            ...
            onImportReminders([]); // Clear imported reminders
        }
    };
    ...
    // Update whenever selection changes using cached data
    useEffect(() => {
        if (importReminders && allRawReminders.length > 0) {
            processAndSync(allRawReminders, selectedListIds);
        }
    }, [selectedListIds]);
                                ...
                                >
                                    <div style={{ padding: '16px 0', borderTop: '1px solid var(--border-color)' }}>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                            ...
                                            {iosLists.length === 0 && !isImporting && <div style={{ padding: '10px', color: 'gray', fontSize: '12px' }}>No lists found. Make sure you have reminders in your Apple Reminders app.</div>}
                                            {isImporting && <div style={{ padding: '10px', color: 'var(--primary)', fontSize: '12px' }}>Fetching lists...</div>}
                                        </div>
                                        <div style={{ marginTop: '16px', fontSize: '12px', color: 'var(--text-muted)', fontStyle: 'italic', padding: '10px 0' }}>
                                            Syncing Mac Reminders to Tiimo...
                                        </div>
                                    </div>
                                </motion.div>
                            )}
